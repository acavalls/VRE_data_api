...
class TokenVerify extends Middleware {
    ...
    // get mug_id from resource owner info
    $mug_id = $this->oauth2->getAttrFromTokenResponse("mug_id",$r);
    if (!$mug_id){
		$code = 406;
		$errMsg = "Token successfully authorized, but resource onwer retrieval failed. No 'mug_id'";
        throw new \Exception($errMsg, $code);
    }
   	$request = $request->withAttribute('mug_id', $mug_id);

    // get username. Having mug_id, NOT REQUIRED ANYMORE  
	$userLogin = $this->oauth2->getAttrFromTokenResponse("username",$r);
	if (empty($userLogin)){
		$code = 406;
		$errMsg = "Token successfully authorized, but resource onwer retrieval failed. No 'username'";
		throw new \Exception($errMsg, $code);
	}
